...
argparser.add_argument(
    ...
    type=str,
    default="navset",
    help="Train dataset. Can be any one of: 'stepgame', 'navset' or 'trivia'.",
)
...
lora_model_name = f"{model_filename}_{args.train_dataset}_lora"
...
selected_data = dataset[dataset["split"] == "test"]
train_dataset = dataset[dataset["split"] == "train"]
ref_scenario = train_dataset['text'].tolist()
ref_fact = train_dataset['facts'].tolist()


columns = ['text', 'label', 'facts', 'output']
results_df = pd.DataFrame(columns = columns)


save_path = f"results/navset_{lora_model_name}_shot_{args.shot}_{args.shot_strategy}_test.tsv"
...
for i in range(len(results_df), len(selected_data)):
    ...
    system_prompt = "Your task is to convert the scenario into factual statements. Based on the first vessel, you will describe the features and direction of all vessels. Unless otherwise specified, assume that each vessel is powered by an engine."
    query = "Convert the above scenario into the following facts: starboard, port, head-on, engine, sail, restricted."
    ...
    row_result = pd.DataFrame({"text": scenario, "label": label, "facts":fact, 
