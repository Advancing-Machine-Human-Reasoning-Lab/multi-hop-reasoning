...
new_model = f"{model_filename}_stepgame_lora"
...
dataset = pd.read_csv('train_facts.tsv', delimiter='\t')
dataset = Dataset.from_dict({"input_text": dataset['Text'], "labels": dataset['Facts']})
...
def format_chat_template(row):
    ...
    story = row["input_text"]
    user_prompt = f"Scenario: {story}\n{query}"
    facts = row["labels"]
    out1 = f"Facts:\n{facts}"
